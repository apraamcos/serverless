...
class AwsCompileMSKEvents {
  ...
  compileMSKEvents() {
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      let count = 0;
      let dependsOnId;
      for (const event of functionObj.events) {
        if (event.msk) {
          ...
          console.info(mskEventLogicalId)

          if(count && (count % 29 === 0)) {
            dependsOnId = mskEventLogicalId;
            console.info(count, mskEventLogicalId)
          }

          const dependsOn = [
            dependsOnId || (this.provider.resolveFunctionIamRoleResourceName(functionObj),
            _.get(functionObj.targetAlias, 'logicalId')),
          ].filter(Boolean);

          console.info(dependsOn)
      ...
      console.info(cfTemplate.Resources);      
